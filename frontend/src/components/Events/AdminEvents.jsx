...
function AdminEvent() {
  ...
  const addEvent = async (formData) => {
    try {
      ...
      await axios.post("http://localhost:3001/event/addEvent", formData);
  ...
  const deleteEvent = async (_id) => {
    try {
      await axios.delete(`http://localhost:3001/event/deleteEvent/${_id}`, {
  ...
  const downloadPdfReport = () => {
    ...
    autoTable(doc, {
      theme: "striped",
      head: [["Event ID", "Event Titlee", "Description", "Date"]],
      body: data.map((item) => [
        item.eventId,
        item.eventTitle,
        item.description,
